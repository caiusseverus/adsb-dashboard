...
function RotationAlignmentPanel({
  ...
  const visibleDf11ResidualDots = syncDrivingOnlyMode === 'sync' ? [] : (showDf11Overlay === 'show' ? df11ResidualDots : [])
    ...
    <section className={styles.card} data-panel="rotation-alignment">
      <div className={styles.cardHeader}>
        ...
        <div className={styles.metricRow}>
          ...
          {alignmentMode === BURST_SYNC_VIEW_MODE_RESIDUALS && (
            <span className={styles.metricPill} title="When sync-driving only, only burst-centre observations with sync_update_eligible=true AND fit_eligible=true are plotted. DF11 overlay dots are hidden. Excluded burst diagnostics have different opacity and stroke in all-diagnostics mode.">
              Sync-driving only
              <select
                value={syncDrivingOnlyMode}
                onChange={e => setSyncDrivingOnlyMode(e.target.value)}
                style={{
                  background: 'transparent',
                  border: '1px solid #30363d',
                  borderRadius: '3px',
                  color: syncDrivingOnlyMode === 'sync' ? '#58a6ff' : '#c9d1d9',
                  fontSize: '0.72rem',
                  marginLeft: '4px',
                  padding: '1px 3px',
                }}
              >
                ...
                <option value="sync">Sync-driving only</option>
              </select>
            </span>
          )}
          {alignmentMode === BURST_SYNC_VIEW_MODE_RESIDUALS && (
            <span className={styles.metricPill} title="Show individual DF11 reply residual dots as an overlay on the burst-centre chart.">
              ...
              <select
                ...
                onChange={e => setShowDf11Overlay(e.target.value)}
                style={{
                  background: 'transparent',
                  border: '1px solid #30363d',
                  borderRadius: '3px',
              ...
              >
                <option value="show">Show DF11 dots</option>
                <option value="hide">Hide DF11 dots</option>
              </select>
            </span>
          )}
          {alignmentMode === BURST_SYNC_VIEW_MODE_RESIDUALS && (
          ...
          {filteredObservations.length === 0 && visibleDf11ResidualDots.length === 0 ? (
            ...
            <>
              ...
              <div style={{ fontSize: '0.74rem', color: '#8b949e', marginTop: '0.5rem', textAlign: 'center' }}>
                ...
                {' · '}Sync-driving fit points {syncDrivingCount}
                {nonSyncDrivingCount > 0 ? ` · Excluded burst diagnostics ${nonSyncDrivingCount}` : ''}
                ...
                {residualChartMode === RESIDUAL_CHART_MODE_RECORDED && recordedEventDiagnostics?.recorded_event_count_total != null
                  ...
                  : ''}
                {syncDrivingOnlyMode !== 'sync' ? (
                  <>
                    {' · '}Full: inlier {fullInlierCount} · soft {fullSoftCount} · rejected {fullRejectedCount}
                    {' · '}sync-fit {fullSyncFitCount} · excluded diagnostics {fullNonSyncBurstCount}
