...
def test_radar_page_uses_anchor_relative_phase_wording():
    ...
    assert "Geographic radar beam direction is not known" in text
    assert "Phase trust status:" not in text
    assert "Geographic phase" in text
    assert "Sweep-relative phase" in text
    assert "Phase unavailable" in text
