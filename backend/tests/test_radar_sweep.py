...
def test_burst_residual_recorded_events_are_immutable_across_period_change(monkeypatch):
    ...
    assert recorded[0]["event_time_basis"] == "beam_center_us"
    assert recorded[0]["residual_basis"] == "runtime_effective"
    assert recorded[0]["residual_basis_period_s"] == recorded[0]["event_effective_period_s"]
    assert recorded[0]["residual_basis_phase_offset_deg"] == recorded[0]["event_phase_offset_deg"]
    assert recorded[0]["residual_basis_authority"] == recorded[0]["event_period_authority"]
    assert recorded[0]["classifier_input"] is True
    assert recorded[0]["chart_only_diagnostic"] is False
    assert "sync_seq" in recorded[0]
    assert "dominant_family_id" in recorded[0]
    assert "secondary_family_id" in recorded[0]
    assert "reference_icao" in recorded[0]
    assert "anchor_icao" in recorded[0]
    assert "anchor_status" in recorded[0]
    assert "anchor_trust" in recorded[0]
    ...
    assert recorded[0]["event_effective_period_s"] == pytest.approx(4.0)
    assert recorded[0]["event_phase_offset_deg"] != pytest.approx(recorded[1]["event_phase_offset_deg"])


def test_recorded_residual_event_chart_only_and_null_optional_fields():
    state = RadarState()
    sync_snapshot = {
        "base_period_s": 4.0,
        "period_delta_s": 0.0,
        ...
        "phase_offset_basis": "relative",
        "phase_epoch_us": 1000.0,
        "handoff_state": "GO_REFINING",
        "handoff_reason": "strict_gate_failed",
        "period_refinement_status": "insufficient_history",
        "contamination_state": "insufficient_data",
        "contamination_reason": "insufficient_data",
        "contamination_gate_reason": "insufficient_data",
    }
    row = state._build_recorded_residual_event(
        iid=1,
        icao="ABC123",
        wall_ts=1000.0,
        arrival_beast_us=1_000_000.0,
        beam_center_us=1_000_000.0,
        centroid_timestamp_us=1_000_000.0,
        residual_deg=55.0,
        ...
        timing_class=None,
        event_kind="burst",
        corrected_residual_deg=55.0,
        pos_age_s=0.5,
        dominant_family=False,
    )
    assert row["classifier_input"] is False
    assert row["chart_only_diagnostic"] is True
    assert row["sync_update_eligible"] is False
    assert row["fit_eligible"] is False
    assert row["hard_reject_reason"] == "wrapped_abs_residual_gt_50deg"
    assert row["soft_reason"] is None
    assert "secondary_family_id" in row and row["secondary_family_id"] is None
    assert "reference_icao" in row and row["reference_icao"] is None
    assert "anchor_icao" in row and row["anchor_icao"] is None
...
def test_go_sync_snapshot_without_live_sync_state_serializes_nonactive_go_blocker(monkeypatch):
    ...
    assert sync_state["readiness_state"] == "sync_unusable"
    assert sync_state["readiness_reason"] == "strict_gate_failed"
...
def test_go_sync_snapshot_without_live_sync_state_ready_like_maps_to_hysteresis(monkeypatch):
    ...
    assert sync_state["readiness_state"] == "hysteresis_not_started"
    assert sync_state["readiness_reason"] == "go_ready_hysteresis_not_started"
...
def test_holdover_reacquire_trace_first_failed_stale_ref_position():
    ...
    assert payload["reacquire_ref_selection_source"] == "fit_history"
    assert payload["reacquire_ref_icao_fit_pool_present"] is True
    assert payload["reacquire_ref_icao_support_pool_present"] is True
...
def test_holdover_reacquire_reference_selection_and_shadow_fields_passthrough():
    sync = LiveSyncState(
        iid=405,
        period_s=4.0,
        period_base_s=4.0,
        ...
        holdover=True,
        holdover_reason="stale_reference_position",
        holdover_entered_ts=time.time() - 8.0,
        holdover_last_transition_ts=time.time() - 8.0,
        last_update_epoch_ref_icao="ABC123",
        last_update_epoch_ref_pos_age_s=12.0,
        last_update_epoch_n_aircraft=2,
    )
    payload = sweep._live_sync_state_to_dict(
        sync,
        go_sync={
            ...
            "reacquire_better_ref_candidate_pos_age_s": 1.7,
    ...
    assert payload["reacquire_better_ref_candidate_available"] is True
    assert payload["reacquire_better_ref_candidate_icao"] == "DEF456"
    assert payload["shadow_reacquire_with_best_fresh_ref"] is True
    assert payload["shadow_ref_candidate_icao"] == "DEF456"
    assert payload["shadow_all_reacquire_gates_pass_with_candidate"] is False
...
def test_stage10_operational_invariants_when_go_active(monkeypatch):
    """When go_operational_active=True, authority and period-source invariants hold."""
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    state = _make_state_with_stable_history(1801, period_s=4.0)
    state.update_go_iid_state({
        "i": 1801, "sp": True, "su": True, "sps": 4.001, "sep": 1000.0, "sod": 10.0,
        "sq": 0.9, "sj": 1.0, "snf": 10, "sh": False, "lu": 2000.0, "rv": 1,
        "bps": 4.0, "eps": 4.001, "pag": True, "pds": 0.001,
    })
    _drive_go_handoff_cycles(state, 1801, cycles=3)
    payload = sweep._live_sync_state_to_dict(
        state.get_live_sync_state(1801),
        go_sync=state.get_go_live_sync_state(1801) or {},
    )
    assert payload["go_operational_active"] is True
    assert payload["sync_authority"] == "go_runtime"
    assert payload["period_authority"] == "go_refined"
    assert payload["effective_period_source"] == "go_runtime.effective_period_s"
    assert payload["operational_period_s"] == payload["effective_period_s"]
    assert payload["python_base_period_s"] is not None
    assert payload["go_refined_period_delta_s"] is not None
    assert abs(
        (float(payload["python_base_period_s"]) + float(payload["go_refined_period_delta_s"]))
        - float(payload["operational_period_s"])
    ) < 1e-9


def test_stage10_operational_invariants_when_go_not_active(monkeypatch):
    """When go_operational_active=False, operational period falls back to Python base."""
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", False)
    state = _make_state_with_stable_history(1802, period_s=4.0)
    state.update_go_iid_state(_go_sync_for_gates(1802, 4.0))
    payload = sweep._live_sync_state_to_dict(
        state.get_live_sync_state(1802),
        go_sync=state.get_go_live_sync_state(1802) or {},
    )
    assert payload["go_operational_active"] is False
    assert payload["period_authority"] == "py_base"
    assert payload["sync_authority"] == "py_bootstrap"
    assert payload["operational_period_s"] == payload["python_base_period_s"]
    assert payload["effective_period_source"] == "python_simple_sync.period_base_s"
    assert payload["go_refined_period_delta_s"] is None
...
def test_stage10_serializer_flag_disabled_keeps_diagnostic_shadow_fallback(monkeypatch):
    """With operational flag disabled, legacy diagnostic fallback labels remain allowed."""
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", False)
    from radar.sync_models import LiveSyncState
    sync = LiveSyncState(
    ...
    assert payload["go_refiner_operational_enabled"] is False
    assert payload["handoff_state"] == "UNTRUSTED"
    assert payload["handoff_reason"] == "go_diagnostic_shadow"


def test_stage10_serializer_enabled_phase_readiness_failure_sets_prefixed_blocking_gate(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    from radar.sync_models import LiveSyncState
    sync = LiveSyncState(
        ...
        handoff_gate_failures={
            "python_base": {"enough_icaos": {"passed": True, "reason": None}},
            "go_readiness": {"go_state_present": {"passed": True, "reason": None}},
            "phase_readiness": {"phase_basis_supported": {"passed": False, "reason": "phase_basis_not_supported"}},
        },
    )
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["go_refiner_operational_enabled"] is True
    assert payload["go_operational_active"] is False
    assert payload["blocking_gate"] == "phase_readiness.phase_basis_supported"
    assert payload["handoff_reason"] == "phase_basis_not_supported"
    assert payload["handoff_reason"] != "go_diagnostic_shadow"


def test_stage10_serializer_enabled_go_readiness_failure_sets_prefixed_blocking_gate(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    from radar.sync_models import LiveSyncState
    sync = LiveSyncState(
        ...
        handoff_gate_failures={
            "python_base": {"enough_icaos": {"passed": True, "reason": None}},
            "go_readiness": {"go_sync_state_usable": {"passed": False, "reason": "go_sync_unusable"}},
            "phase_readiness": {"phase_basis_supported": {"passed": True, "reason": None}},
        },
    )
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["go_refiner_operational_enabled"] is True
    assert payload["go_operational_active"] is False
    assert payload["blocking_gate"] == "go_readiness.go_sync_state_usable"
    assert payload["handoff_reason"] == "go_sync_unusable"
    assert payload["handoff_reason"] != "go_diagnostic_shadow"


def test_stage10_no_untrusted_diagnostic_shadow_with_missing_blocking_gate_when_enabled(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    from radar.sync_models import LiveSyncState
    sync = LiveSyncState(
        ...
        handoff_reason="",
        handoff_gate_failures={
            "python_base": {"enough_icaos": {"passed": False, "reason": "insufficient_contributing_icaos"}},
            "go_readiness": {},
            "phase_readiness": {},
        },
    )
    ...
    assert payload["go_operational_active"] is False
    assert payload["handoff_state"] != "GO_REFINED_READY"
    assert payload["blocking_gate"] is not None
    assert payload["handoff_reason"] != "go_diagnostic_shadow"


def test_stage10_enabled_nonactive_without_gate_details_gets_unclassified_blocking_gate(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    from radar.sync_models import LiveSyncState
    sync = LiveSyncState(
    ...
    assert payload["go_refiner_operational_enabled"] is True
    assert payload["go_operational_active"] is False
    assert payload["blocking_gate"] == "go_readiness.go_refinement_history_sufficient"
...
def test_stage10_enabled_nonactive_uses_go_sync_unusable_reason_before_unclassified(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    from radar.sync_models import LiveSyncState
    sync = LiveSyncState(
...
def test_stage10_enabled_nonactive_stable_usable_without_go_sync_dict_maps_to_hysteresis_not_started(monkeypatch):
    ...
    sync.go_sync_usable_quality_ok = True
    sync.go_sync_usable_holdover_ok = True
    sync.go_sync_usable_period_agrees = True
    sync.go_sync_usable_strict_gate_pass = True
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["go_operational_active"] is False
    assert payload["go_operational_blocking_gate"] == "go_readiness_hysteresis"
    assert payload["handoff_reason"] == "go_ready_hysteresis_not_started"
...
def test_stage10_enabled_nonactive_stable_usable_int_flags_map_to_hysteresis_not_started(monkeypatch):
    ...
    payload = sweep._live_sync_state_to_dict(
        ...
        go_sync={
            "period_refinement_status": "stable",
            "go_sync_usable_quality_ok": 1,
            "go_sync_usable_holdover_ok": 1,
            "go_sync_usable_period_agrees": 1,
            "go_sync_usable_strict_gate_pass": 1,
        },
    )
    assert payload["go_operational_active"] is False
    assert payload["go_operational_blocking_gate"] == "go_readiness_hysteresis"
...
def test_stage10_live_unclassified_shape_with_diag_usable_flags_maps_to_hysteresis_not_started(monkeypatch):
    ...
    sync = LiveSyncState(
        iid=28,
        period_s=4.084356826834769,
        period_base_s=4.084356826834769,
        phase_epoch_us=41098203.22979099,
        ...
        usable=True,
        holdover=False,
        holdover_reason=None,
        period_authority="py_base",
        sync_authority="py_bootstrap",
        handoff_state="GO_REFINING",
        handoff_reason="go_state_unclassified",
        handoff_gate_failures={},
        fit_total_observations=13,
        fit_icao_count=0,
        contamination_state="not_evaluated",
        population_validation_state="pass",
        period_refinement_status="stable",
    ...
    sync.go_diagnostic_holdover = False
    sync.go_diagnostic_holdover_reason = ""
    ...
    sync.go_diagnostic_fit_observation_count = 5
    sync.go_diagnostic_fit_icao_count = 4
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["go_operational_blocking_gate"] == "go_readiness_hysteresis"
...
def test_stage10_live_captured_unclassified_shape_without_go_sync_maps_to_hysteresis_not_started(monkeypatch):
    ...
    sync = LiveSyncState(
        ...
        phase_anchor_icao="ABC123",
        phase_anchor_status="selected",
        phase_anchor_since_ts=time.time() - 1.0,
    )
    sync.go_sync_unusable_reason = None
    sync.go_diagnostic_go_sync_unusable_reason = ""
    sync.go_diagnostic_refinement_status = "stable"
...
def test_stage10_active_go_with_phase_failure_uses_phase_blocker_not_go_blocker(monkeypatch):
    ...
    sync = LiveSyncState(
        ...
        handoff_gate_failures={
            "python_base": {"enough_icaos": {"passed": True, "reason": None}},
            "go_readiness": {"go_sync_state_usable": {"passed": True, "reason": None}},
            "phase_readiness": {"phase_basis_supported": {"passed": False, "reason": "phase_basis_not_supported"}},
        },
        phase_blocking_gate="phase_basis_supported",
        phase_blocking_reason="phase_basis_not_supported",
    ...
    assert payload["handoff_state"] == "GO_REFINED_READY"
    assert payload["handoff_reason"] == "go_runtime_operational"
    assert payload["blocking_gate"] is None
    assert payload["go_operational_blocking_gate"] is None
    assert payload["phase_authority_blocking_gate"] == "phase_readiness.phase_basis_supported"
    assert payload["phase_authority_blocking_reason"] == "phase_basis_not_supported"


def test_stage10_readiness_state_operational(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    sync = LiveSyncState(
        iid=19001,
    ...
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["readiness_state"] == "operational"
    assert payload["readiness_reason"] == "go_runtime_operational"


def test_stage10_readiness_state_holdover(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    sync = LiveSyncState(
        iid=19002,
    ...
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["readiness_state"] == "holdover"
    assert payload["readiness_reason"] in {"go_holdover", "insufficient_aircraft", "reacquire_valid_ref_icao"}


def test_stage10_readiness_state_sync_unusable(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    sync = LiveSyncState(
        iid=19003,
    ...
    sync.go_sync_unusable_reason = "quality_below_threshold"
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["readiness_state"] == "sync_unusable"
    assert payload["readiness_reason"] == "quality_below_threshold"


def test_stage10_readiness_state_hysteresis_not_started(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    sync = LiveSyncState(
        iid=19004,
    ...
    assert payload["readiness_state"] == "hysteresis_not_started"
    assert payload["readiness_reason"] == "go_ready_hysteresis_not_started"


def test_stage10_readiness_state_contamination_blocked(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    sync = LiveSyncState(
        iid=19005,
        ...
        usable=False,
        handoff_state="GO_REFINING",
        handoff_reason="contamination_detected",
        blocking_gate="go_readiness.contamination_free",
        contamination_state="contaminated",
    )
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["readiness_state"] == "contamination_blocked"
    assert payload["readiness_reason"] == "contamination_detected"


def test_stage10_readiness_state_phase_not_ready_not_go_blocker_when_active(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    sync = LiveSyncState(
        iid=19006,
...
def test_stage10_readiness_state_unknown_only_for_unknown_shapes(monkeypatch):
    import config as _cfg
    monkeypatch.setattr(_cfg, "RADAR_SYNC_GO_REFINER_OPERATIONAL", True)
    sync = LiveSyncState(
        iid=19007,
        ...
        blocking_gate="go_readiness.custom_debug_gate",
    )
    payload = sweep._live_sync_state_to_dict(sync)
    assert payload["readiness_state"] == "unknown"
    assert payload["readiness_reason"] == "mystery_blocker"
...
def test_fresh_go_evidence_without_burst_rows_uses_display_absence_reason():
    state = RadarState()
    iid = 9912
    now = time.time()
    state._go_evidence_events.append({"iid": iid, "wall_ts": now - 1.0})
    state.update_go_iid_state({
        "i": iid, "sp": True, "su": True, "sps": 4.0, "sep": 2100.0, "sod": 15.0,
        "sq": 1.0, "sj": 2.0, "sh": False, "lu": now, "rv": 1, "bps": 4.0, "eps": 4.0,
        "foc": 12, "fic": 4, "pag": True,
